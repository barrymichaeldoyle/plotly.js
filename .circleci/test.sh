...
case $1 in
    ...
    virtual-webgl-jasmine)
        ...
        for s in ${SHARDS[@]}; do
            MAX_AUTO_RETRY=2
            retry ./node_modules/karma/bin/karma start test/jasmine/karma.conf.js --virtualWebgl --tags=gl --skip-tags=noCI --doNotFailOnEmptyTestSuite -- "$s"
        done
    ...
    make-baselines-virtual-webgl)
        SUITE=$(find $ROOT/test/image/mocks/gl* -type f -printf "%f\n" | sed 's/\.json$//1' | circleci tests split)
